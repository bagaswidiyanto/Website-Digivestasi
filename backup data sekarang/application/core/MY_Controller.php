...
class MY_Controller extends CI_Controller
{
     ...
     public function layout()
     {
          ...
          $this->data['website_ident'] = $this->CI->db->get('tbl_website')->row();
     ...
     //   {
     //        $get_res = $this->db->query("SELECT DISTINCT count(ip) as jml FROM tbl_counter where date(`timestamp`)=CURDATE() AND location LIKE '/index.php/%' ");

     //   }else if($mode=="online"){
     //        $get_res = $this->db->query("SELECT DISTINCT count(ip) as jml FROM tbl_counter where `timestamp` < DATE_ADD(NOW(), INTERVAL -10 minute) AND date(`timestamp`)=CURDATE() AND location LIKE '/index.php/%' ");
     //   }     else{ // query perhitungan seluruh IP Address (tidak unik)
     //        $get_res = $this->db->query("SELECT count(ip) as jml FROM tbl_counter WHERE location LIKE '/index.php/%' ");
     //   }
