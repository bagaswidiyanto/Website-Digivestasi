...
class GetListBerita extends MY_Controller {
  ...
  public function index()
  {
    ...
    $config['base_url'] = base_url().'getlistberita/index/';
  ...
  public function tag(){
    $this->data['listberita']=$this->input->get('id');
    ...
    $config['base_url'] = base_url().'getlistberita/list/'.$tag.'/index/';
    $config['uri_segment'] = 3;
    $config['total_rows'] = $jml;
    $config['per_page'] = 6;
    $config['full_tag_open'] = '<center><ul class="pagination pagination-sm no-margin justify-content-center">';
  ...
  public function search(){
    ...
    $search=@$_GET['search'];
    $this->data['post']=$this->db->query('SELECT * from tbl_posts where (Title like "%'.$search.'%" OR content like "%'.$search.'%") ORDER BY created_date desc');
    $this->web='content/v_get_search';
    $this->layout();
  }
