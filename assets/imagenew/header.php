...
<head>
    ...
    <?php if ($this->uri->segment(2) != 'detail') { ?>
        ...
        <meta property="og:image" itemprop="image" content="<?= base_url(); ?>assets/imagenew/logo_link.png">
                ...
                <div class="col-lg">
                    <marquee direction="" onmouseover="this.stop();" onmouseout="this.start();">
                        <?php foreach ($this->db->query("SELECT * FROM tbl_posts order by id DESC ")->result() as $berita) { ?>
                            <a href="<?= base_url() . 'news/detail/' . $berita->slug ?>"><small>🔥 <?= $berita->Title; ?></small></a>
                        <?php } ?>
                    </marquee>
                </div>
            ...
            <div class="row align-items-center justify-content-between gx-0 d-flex">
                ...
                <div class="col-lg-4 col-sm-7 col-7 text-center">
                    ...
                    if ($this->session->userdata('user_id')) { ?>
                        <a href="<?= base_url('logout') ?>" class="login btn bg-white text-dark rounded-10 px-3 py-2">Logout</a>
                    <?php } else { ?>
                        <a href="<?= base_url('login') ?>" class="login btn bg-white text-dark rounded-10 px-3 py-2">Login</a>
                    <?php } ?>
                </div>
            </div>
        </div>
